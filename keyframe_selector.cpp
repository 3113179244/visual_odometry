...
void KeyframeSelector::setParameters(double near_depth_thresh, double loss_ratio_thresh, int min_near_points,
    ...
    near_depth_thresh_ = near_depth_thresh; loss_ratio_thresh_ = loss_ratio_thresh; min_near_points_ = min_near_points;
    min_avg_disparity_ = min_avg_disparity; trans_base_ratio_ = trans_base_ratio; grid_rows_ = grid_cell_rows;
    grid_cols_ = grid_cell_cols; coverage_ratio_ = coverage_ratio; redundancy_ratio_ = redundancy_ratio;
}
...
double KeyframeSelector::computeAverageDisparity(const Keyframe& last_kf, const std::vector<cv::Point2f>& curr_kps, const std::vector<int>& matched_ids) const {
    ...
    for (size_t i = 0; i < last_kf.map_point_indices.size(); ++i) {
        ...
        if (mp_idx >= 0) last_kf_map[mp_idx] = last_kf.keypoints_2d[i];
    }
    double sum_disparity = 0.0; int valid_count = 0;
    for (size_t i = 0; i < matched_ids.size() && i < curr_kps.size(); ++i) {
        int mp_idx = matched_ids[i]; auto it = last_kf_map.find(mp_idx);
        if (it != last_kf_map.end()) {
            cv::Point2f last_pt = it->second; cv::Point2f curr_pt = curr_kps[i];
            double dx = last_pt.x - curr_pt.x; double dy = last_pt.y - curr_pt.y;
            sum_disparity += std::sqrt(dx*dx + dy*dy); valid_count++;
        }
    }
...
double KeyframeSelector::computeGridCoverage(const std::vector<cv::Point2f>& keypoints_2d) const {
    ...
    int total_cells = grid_rows_ * grid_cols_; std::vector<bool> covered(total_cells, false);
    double cell_w = static_cast<double>(img_width_) / grid_cols_; double cell_h = static_cast<double>(img_height_) / grid_rows_;
    for (const auto& pt : keypoints_2d) {
        int col = static_cast<int>(pt.x / cell_w); int row = static_cast<int>(pt.y / cell_h);
        if (col >= 0 && col < grid_cols_ && row >= 0 && row < grid_rows_) covered[row * grid_cols_ + col] = true;
    }
    ...
    for (int mp_idx : matched_mp_indices) {
        ...
        for (const auto& kf : window_keyframes) {
            for (int kf_mp_idx : kf.map_point_indices) {
                if (kf_mp_idx == mp_idx) { observed_elsewhere.insert(mp_idx); break; }
            }
        }
    }
...
bool KeyframeSelector::decide(const cv::Mat& curr_rvec, const cv::Mat& curr_tvec,
                              ...
                              bool is_first_frame) {
    if (is_first_frame || last_keyframe == nullptr) return true;
    cv::Mat R_curr; cv::Rodrigues(curr_rvec, R_curr);
    ...
    cv::Mat R_last; cv::Rodrigues(last_keyframe->rvec, R_last);
    ...
    for (int mp_idx : near_point_indices_curr) { if (last_near_indices.count(mp_idx)) last_near_reobserved++; }
    ...
    if (force_insert_by_low_near || force_insert_by_loss) {
        if (computeRedundancyRatio(matched_mp_indices, window_keyframes) > redundancy_ratio_) return false;
        return true;
    }
    ...
    if (avg_disparity < min_avg_disparity_ && trans_ratio < trans_base_ratio_) return false;
    
    // ========== 4. 网格覆盖度与冗余度 ==========
    if (computeGridCoverage(curr_keypoints_2d) < coverage_ratio_) return false;
    if (computeRedundancyRatio(matched_mp_indices, window_keyframes) > redundancy_ratio_) return false;
