#include "Tracking.h"                 // 引入前端追踪类的头文件，包含本类的声明与成员变量定义
#include "Map.h"                      // 引入地图管理类的头文件，用于向全局地图存取关键帧和路标点
#include "FeatureDetector.h"          // 引入特征处理器类的头文件，用于执行光流、PnP和三角化算法
#include "KeyFrame.h"                 // 引入关键帧类的头文件，用于实例化关键帧数据结构
#include "Optimizer.h"                // 引入后端优化器类的头文件，用于调用 Ceres 实施图优化
#include "Parameters.h"               // 引入全局静态参数类的头文件，用于读取 YAML 配置文件中的配置项
